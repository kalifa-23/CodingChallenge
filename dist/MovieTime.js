...
const movieTime = () => {
    // Ask for user input
    const movies = parseInt(prompt("Enter number of movies: "), 10);
    const minutesEach = parseInt(prompt("Enter minutes per movie: "), 10);
    // Validate input
    if (isNaN(movies) || movies <= 0) {
        console.log("Invalid input: number of movies must be a positive whole number.");
        return;
    }
