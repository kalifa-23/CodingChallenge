...
const bookReadingTracker = (): void =>{
    // Prompt Jay for the numbers of a page in the book
    const pages: number = parseInt(prompt('How many pages in the book? '), 10);

    // Prompt Jay How many pages does he read each day?
    const pagePerDay: number = parseInt(prompt('How many pages does Jay read each day? '), 10);
