...
const speedFineCalculator = (): void => {
  const limit: number = parseInt(prompt("Enter the speed limit: "));
  const speed: number = parseInt(prompt("Enter Mia's speed: "));
  const limitCharge = 5;
  ...
  if (speed > limit) {
    ...
    const fine: number = (speed - limit) * limitCharge;
